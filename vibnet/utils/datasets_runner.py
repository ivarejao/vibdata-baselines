...
from vibdata.deep.signal.transforms import Transform, Sequential
...
def parse_args():
    ...
    parser.add_argument(
        "--datasets",
        help="Define the dataset which will run, if no one is specified all datasets will run.",
    )
...
def convert_deep_datasets(
    raw_datasets: Dict[str, RawVibrationDataset],
    root_dir: Path,
...
def instantiate_deep_datasets(
    root_dir: Path, transforms: Union[Sequential, None], names: List[str]
) -> List[DeepDataset]:
    ...
    for dt_path in stored_datasets:
        deep_datasets.append(DeepDataset(dt_path, transforms))
...
def main():
    ...
    raw_datasets = load_raw_datasets(datasets_names, root_dir)
    ...
    convert_deep_datasets(raw_datasets, root_dir)
    ...
    for name, dataset in zip(datasets_names, deep_datasets):
        ...
        with tqdm(total=len(loader), desc="Running: ") as pb:
            for ret in loader:
                sizeof = lambda x: 1 if isinstance(x["metainfo"], pd.Series) else x["signal"].shape[0]  # noqa
    ...
    pd.DataFrame(samples).to_csv("samples_per_datasets_transformed.csv", index=False)
