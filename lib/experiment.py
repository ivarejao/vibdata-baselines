...
def _log_parameters(config: dict = None):
    """Logs the transformations' parameters to WANDB.

    Args:
        parameters (dict): experiment configs.
    """

    exp_cfgs = {}
    exp_cfgs.update({"transforms": str(config["transforms"])})
    for key, items in config["params"].items():
        for i, j in items.items():
            exp_cfgs.update({f"{key}_{str(i)}": str(j)})
...
class Experiment:
    def __init__(self, exp_name, output_dir: str = "output"):
        ...
        load_dotenv(dotenv_path="/home/ngmonteiro/experiments/vibnet-mod/wandb.env")
    ...
    def configure_wandb(self, run_name: str) -> None:
        ...
        configs = {
            ...
            "learning_rate": (self.cfg["optimizer"]["parameters"]["lr"] if "optimizer" in self.cfg.config else "None"),
            "weight_decay": (
